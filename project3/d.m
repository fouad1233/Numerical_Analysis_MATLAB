...
y_c = cubicSpline(@p  ,fi, [0,2*pi] , 360);
xh =@(fi) y_c .*cos(fi);
yh =@(fi) y_c .*sin(fi); 
...
%%
h= fi(2);
g_f_h = ( y_c(2:360) - y_c(1:359)  )./h;

g_f = forward_difference_deritave(@p,fi,fi(2));
nexttile
polarplot(fi(1:359), g_f_h)
hold on
polarplot(fi , g_f,"*",'MarkerIndices',1:2:length(ly(@L, dxfi_dfi_f,dyfi_dfi_f)) )
...
hold on 
polarplot(lx(@L,dxfi_dfi_b, dyfi_dfi_b), ly(@L, dxfi_dfi_b,dyfi_dfi_b) ,"*",'MarkerIndices',1:5:length(ly(@L, dxfi_dfi_f,dyfi_dfi_f)))

title("lx and ly back difference")
%%
%%%%%%%%%%%%%%%
h= fi(2);
g_b_h = (  y_c(1:359) - y_c(2:360)  )./ -h;
g_b = back_difference_deritave(@p,fi,fi(2));
nexttile
polarplot(fi(2:360), g_b_h)
hold on 
polarplot(fi, g_b,"*",'MarkerIndices', 1:2:360 )
...
hold on
polarplot(lx(@L,dxfi_dfi_e, dyfi_dfi_e), ly(@L, dxfi_dfi_e, dyfi_dfi_e), "*",'MarkerIndices', 1:4:360)
title("lx and ly three point end point")
%%
%%%%%%%%%%%%%%
h= fi(2);
g_e_h = (1/(2*h)) * (-3.*y_c(1:358) + 4.*y_c(2:359) -y_c(3:360));
g_e = three_point_end_point(@p,fi,fi(2));
nexttile
polarplot(fi(1:358), g_e_h)
hold on 
polarplot(fi, g_e, "*",'MarkerIndices', 1:2:360 )
...
hold on
polarplot(lx(@L,dxfi_dfi_m, dyfi_dfi_m), ly(@L, dxfi_dfi_m, dyfi_dfi_m) ,"*","MarkerIndices",1:4:360 )
title("lx and ly middle point")
%%
%%%%%%%%%%%%
h= fi(2);
g_m_h = (1/(2* h)) * (y_c(3:360) -y_c(1:358) );
g_m = three_point_middle_point(@p,fi,fi(2));
nexttile
polarplot(fi(2:359), g_m_h)
hold on
polarplot(fi, g_m,"*","MarkerIndices",1:4:360)
